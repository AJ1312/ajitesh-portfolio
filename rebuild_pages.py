import os
from bs4 import BeautifulSoup

PAGE_TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{title} | The Sharma Dispatch</title>
    
    <link rel="icon" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>A</text></svg>">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&family=Playfair+Display:ital,wght@0,400;0,600;0,700;0,800;0,900;1,400;1,700&display=swap" rel="stylesheet">
    
    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/gsap.min.js" defer></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/ScrollTrigger.min.js" defer></script>
    <link rel="stylesheet" href="../css/styles.css">
</head>
<body class="newspaper-theme">
    <div class="container">
        
        <!-- SUBPAGE HEADER -->
        <header class="masthead section-rule--double" style="margin-top: 20px;">
            <nav class="nav" id="main-nav" style="border-bottom: 4px solid var(--rule-dark); padding: 15px 0;">
                <div class="newspaper-grid" style="align-items: center;">
                    <div class="col-span-6 heading-md" style="margin:0;"><a href="../index.html" style="text-decoration: none; color: inherit;">&larr; THE SHARMA DISPATCH</a></div>
                    <div class="col-span-6" style="display: flex; justify-content: flex-end; gap: 25px; align-items: center;">
                        <a href="../index.html#projects" class="meta bold">WORK</a>
                        <a href="../index.html#contact" class="meta bold">CONTACT</a>
                        <a href="mailto:13ajitesh@gmail.com" style="background: var(--ink); color: var(--paper); padding: 6px 16px;" class="meta bold">HIRE HIM</a>
                    </div>
                </div>
            </nav>
        </header>

        <main class="dispatch-content">
            {content}
        </main>
        
        <!-- FOOTER -->
        <footer class="section" style="border-top: 4px solid var(--rule-dark); padding: 40px 0;">
            <div class="newspaper-grid">
                <div class="col-span-12 text-center rv">
                    <p class="meta uppercase">&copy; 2026 Ajitesh Sharma &middot; The Sharma Dispatch &mdash; Vol. I</p>
                </div>
            </div>
        </footer>
    </div>
    
    <script src="../js/main.js" defer></script>
    <script src="../js/animations.js" defer></script>
    <script src="../js/simulations.js" defer></script>
</body>
</html>
"""

def extract_text(element):
    if not element: return ""
    return ' '.join(element.stripped_strings)

def extract_html_inner(element):
    if not element: return ""
    return "".join(str(c) for c in element.contents)

def process_file(filepath):
    with open(filepath, 'r') as f:
        html = f.read()
    
    soup = BeautifulSoup(html, 'html.parser')
    
    title = soup.title.string.split('|')[0].strip() if soup.title else "Dispatch"
    
    # Hero Section
    kicker = soup.find(class_=lambda x: x and 'hero-kicker' in x)
    kicker_text = extract_text(kicker) if kicker else "DISPATCH"
    
    hero_title = soup.find(class_=lambda x: x and 'hero-title' in x)
    title_text = extract_text(hero_title) if hero_title else title
    
    hero_subtitle = soup.find(class_=lambda x: x and 'hero-subtitle' in x)
    subtitle_text = extract_text(hero_subtitle) if hero_subtitle else ""

    # Metadata Grid
    meta_html = ""
    meta_section = soup.find(class_=lambda x: x and 'project-metadata' in x)
    if meta_section:
        cards = meta_section.find_all(class_=lambda x: x and 'meta-card' in x)
        meta_html = '<div class="newspaper-grid" style="margin-top: 30px; border-top: 1px solid var(--rule); padding-top: 20px;">'
        for card in cards:
            label = extract_text(card.find(class_=lambda x: x and 'meta-label' in x))
            value = extract_text(card.find(class_=lambda x: x and 'meta-value' in x))
            meta_html += f'<div class="col-span-2 rv column-bordered"><p class="meta" style="margin-bottom: 5px;">{label}</p><p class="body-sm">{value}</p></div>'
        meta_html += '</div>'

    # Content construction
    content = f"""
            <!-- HERO -->
            <section class="section" style="border-bottom: 4px solid var(--rule-dark); padding: 40px 0;">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <p class="kicker-label rv">{kicker_text}</p>
                        <h1 class="display-xl rv" style="margin-bottom: 20px;">{title_text}</h1>
                        <p class="display-sm rv" style="max-width: 800px;">{subtitle_text}</p>
                        {meta_html}
                    </div>
                </div>
            </section>
    """

    # Interactive Simulation
    sim_section = soup.find(class_=lambda x: x and 'project-simulation' in x)
    if sim_section:
        sim_wrapper = sim_section.find(class_=lambda x: x and 'simulation-wrapper' in x)
        if sim_wrapper:
            sim_inner = extract_html_inner(sim_wrapper)
            # Find associated script
            script = sim_section.find('script')
            script_str = str(script) if script else ""
            content += f"""
            <!-- SIMULATION -->
            <section class="section" style="padding: 40px 0; border-bottom: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12 rv" style="background: var(--ink); color: var(--paper); padding: 20px;">
                        <h3 class="meta" style="margin-bottom: 20px; color: var(--paper);">LIVE TELEMETRY</h3>
                        {sim_inner}
                        {script_str}
                    </div>
                </div>
            </section>
            """

    # Incident
    incident_section = soup.find(class_=lambda x: x and 'project-incident' in x)
    if incident_section:
        para = incident_section.find('p')
        if para:
            text = extract_text(para)
            if text:
                dropcap = f'<span class="dropcap">{text[0]}</span>{text[1:]}'
                content += f"""
            <!-- DETAILS -->
            <section class="section" style="border-bottom: 1px solid var(--rule); padding: 40px 0;">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <h3 class="heading-lg" style="margin-bottom: 20px;">The Incident</h3>
                        <p class="body-lg rv">{dropcap}</p>
                    </div>
                </div>
            </section>
                """

    # Engineering / Architecture
    eng_section = soup.find(class_=lambda x: x and 'project-engineering' in x) or soup.find(class_=lambda x: x and 'project-architecture' in x)
    if eng_section:
        content += f"""
            <!-- ENGINEERING -->
            <section class="section" style="padding: 40px 0; border-bottom: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <h3 class="heading-lg" style="margin-bottom: 20px;">The Engineering</h3>
                    </div>
        """
        # Get all sub-sections
        subsections = eng_section.find_all(class_=lambda x: x and 'border-top' in x)
        for sub in subsections:
            title_el = sub.find('h4') or sub.find('h3')
            title_txt = extract_text(title_el) if title_el else ""
            
            p_list = []
            for p in sub.find_all(['p', 'li']):
                p_list.append(f'<p class="body rv" style="margin-bottom: 10px;">&bull; {extract_text(p)}</p>')
            
            content += f"""
                    <div class="col-span-6 rv column-bordered">
                        <h4 class="heading-md" style="margin-bottom: 15px;">{title_txt}</h4>
                        {''.join(p_list)}
                    </div>
            """
        content += """
                </div>
            </section>
        """

    new_html = PAGE_TEMPLATE.replace("{title}", title).replace("{content}", content)
    
    with open(filepath, 'w') as f:
        f.write(new_html)

for filename in os.listdir('pages'):
    if filename.endswith('.html'):
        process_file(os.path.join('pages', filename))
