...
<head>
    ...
    <style>
        ...
        @media (prefers-reduced-motion: reduce) {{
            .packet-dot {{ transition: none !important; }}
            .anim-stage-node {{ transition: none !important; }}
            .anim-row {{ animation: none !important; }}
        }}
    </style>
</head>
...
CIPHERPULSE_CONTENT = """
            ...
            <section class="section" style="border-bottom: 4px solid var(--rule-dark); padding: 40px 0;">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <p class="kicker-label">FIELD DISPATCH &mdash; SYSTEMS ENGINEERING REPORT</p>
                        <h1 class="display-xl" style="margin-bottom: 15px; font-size: 52px;">CipherPulse</h1>
                        <p class="display-sm" style="max-width: 850px; font-style: italic; line-height: 1.4; margin-bottom: 20px;">Multi-Threaded C++ Engine for Encrypted Traffic Intelligence &amp; Line-Rate Inspection</p>
            ...
            <section class="section" style="padding: 35px 0; border-bottom: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <span class="kicker-label" style="color: var(--stamp);">REAL-WORLD SCENARIO</span>
                        <h3 class="heading-lg" style="margin: 10px 0 15px 0;">Detecting Encrypted C2 Channels at Line-Rate</h3>
            ...
            <section class="section" style="padding: 40px 0; border-bottom: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <span class="kicker-label" style="color: var(--stamp);">PIPELINE ANIMATION</span>
                        <h3 class="heading-lg" style="margin: 10px 0 20px 0;">Interactive 5-Tuple Packet Router &amp; JA4+ Fingerprint Simulator</h3>

                        <div class="anim-canvas-box" id="cipherAnimBox">
                            <div style="display:flex; justify-space-between; align-items:center; border-bottom:1px solid #334155; padding-bottom:15px; margin-bottom:20px;">
                                <div>
                                    <span style="color:#38bdf8; font-weight:bold;">LOCK-FREE PACKET INGESTION</span>
                                    <span style="font-size:12px; color:#94a3b8; margin-left:10px;">eBPF / AF_PACKET Ingress</span>
                                </div>
                                <button class="replay-btn" onclick="playCipherAnim()">▶ Replay Packet Routing</button>
                            </div>

                            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:20px;">
                                <!-- Packet Hash Router -->
                                <div style="background:#020617; border:1px solid #1e293b; padding:15px; border-radius:4px;">
                                    <div style="font-size:12px; color:#94a3b8; font-weight:bold; margin-bottom:10px;">5-TUPLE CONSISTENT HASH ROUTER</div>
                                    <div id="pktStream" style="font-size:13px; line-height:1.6;">
                                        [INGRESS] 10.0.4.12:49152 &rarr; 185.220.101.5:443 (TCP)<br>
                                        &rarr; Hash: <code>0x8f2c41a0</code> &rarr; <span style="color:#38bdf8; font-weight:bold;">Worker Thread #2</span>
                                    </div>
                                </div>

                                <!-- JA4+ Fingerprint Matcher -->
                                <div style="background:#020617; border:1px solid #1e293b; padding:15px; border-radius:4px;">
                                    <div style="font-size:12px; color:#94a3b8; font-weight:bold; margin-bottom:10px;">JA4+ TLS CLIENTHELLO FINGERPRINT</div>
                                    <div id="ja4Match" style="font-size:13px; line-height:1.6;">
                                        Cipher Suites Sorted &amp; Hashed:<br>
                                        <code>t13d190800_c84a8b291410</code><br>
                                        <span style="color:#ef4444; font-weight:bold;">[MATCH: CobaltStrike C2 Signature]</span>
                                    </div>
                                </div>
                            </div>

                            <div style="background:#020617; border:1px solid #1e293b; padding:15px; border-radius:4px; font-size:13px;" id="cipherConsole">
                                [IDLE] Press "Replay Packet Routing" or scroll to simulate 5-tuple consistent hashing and JA4+ fingerprint extraction...
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section class="section" style="padding: 40px 0; border-bottom: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        ...
                        <div class="pipeline-step-box">
                            <h4 class="heading-md"><span class="pipeline-step-num">01</span> RAW Packet Ingestion</h4>
                            <p class="body-lg">Captures raw ethernet frames directly from network interfaces or PCAP dumps using eBPF or high-speed AF_PACKET sockets. Reader threads perform zero-copy packet header extraction.</p>
                            <p class="meta" style="margin-top:8px;"><b>Tools:</b> eBPF / AF_PACKET, POSIX threads &middot; <b>Why:</b> Avoids standard socket buffer copy overhead, capturing line-rate headers directly at kernel ingress.</p>
                        </div>

                        <div class="pipeline-step-box">
                            <h4 class="heading-md"><span class="pipeline-step-num">02</span> Consistent 5-Tuple Hashing</h4>
                            <p class="body-lg">Extracts the packet's 5-tuple (source IP, destination IP, source port, destination port, transport protocol) and hashes it deterministically. All packets belonging to the exact same flow always land on the same worker thread queue.</p>
                            <p class="meta" style="margin-top:8px;"><b>Tools:</b> <code>std::hash</code>, custom <code>ThreadSafeQueue</code> &middot; <b>Why:</b> Eliminates cross-thread state synchronization because a single worker thread owns 100% of a flow's lifecycle.</p>
                        </div>

                        <div class="pipeline-step-box">
                            <h4 class="heading-md"><span class="pipeline-step-num">03</span> Lock-Free Fast Path Processing</h4>
                            <p class="body-lg">Worker threads execute inside dedicated processing loops. Each thread maintains an isolated flow table (<code>std::unordered_map&lt;FiveTuple, FlowTracker&gt;</code>), enabling 100% lock-free lookups and state updates on the hot path.</p>
                            <p class="meta" style="margin-top:8px;"><b>Tools:</b> C++17 STL, <code>std::thread</code> &middot; <b>Why:</b> Eliminates mutex lock contention on core packet processing loops, preventing thread stalling during traffic bursts.</p>
                        </div>

                        <div class="pipeline-step-box">
                            <h4 class="heading-md"><span class="pipeline-step-num">04</span> JA4+ Fingerprint Extraction</h4>
                            <p class="body-lg">Parses TLS ClientHello handshakes, extracts cipher suites and extension lists, filters out GREASE noise extensions, and hashes the sorted strings into a stable 12-character JA4 fingerprint using FNV-1a.</p>
                            <p class="meta" style="margin-top:8px;"><b>Tools:</b> Pure C++17, FNV-1a Hash &middot; <b>Why:</b> Identifies client application types (e.g., Cobalt Strike, Sliver, Python requests) even when SNI hostname is encrypted via ECH.</p>
                        </div>

                        <div class="pipeline-step-box">
                            <h4 class="heading-md"><span class="pipeline-step-num">05</span> Active DNS Correlation Cache</h4>
                            <p class="body-lg">Maintains an in-memory cache of IP-to-domain mappings parsed from plaintext DNS response packets, automatically expiring entries using TTL timestamps to label encrypted TLS IP connections with human-readable domains.</p>
                            <p class="meta" style="margin-top:8px;"><b>Tools:</b> Custom DNS Correlator &middot; <b>Why:</b> Provides context for destination IP addresses when ClientHello Encrypted Server Name Indication (ESNI/ECH) conceals hostnames.</p>
                        </div>
                        ...
                        <div class="pipeline-step-box" style="border:none;">
                            <h4 class="heading-md"><span class="pipeline-step-num">07</span> Structured Audit Log &amp; PCAP Output</h4>
                            <p class="body-lg">Outputs filtered threat pcap files and streams structured JSON threat alerts detailing matched JA4 signatures, flow periodicity metrics, and recommended eBPF firewall drop rules.</p>
                            <p class="meta" style="margin-top:8px;"><b>Tools:</b> C++ File I/O, nlohmann::json &middot; <b>Why:</b> Provides structured audit logs easily consumed by enterprise SIEM and eBPF kernel enforcement modules.</p>
                        </div>
                    </div>
                </div>
            </section>
                    ...
                    <div class="col-span-12">
                        ...
                        <table class="tools-table">
                            <thead>
                                <tr>
                                    <th style="width: 220px;">Tool / Library</th>
                                    <th style="width: 260px;">Role in CipherPulse</th>
                                    <th>Design Justification ("Why this, not X?")</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td><b>Pure C++17 STL</b></td>
                                    <td>Core Engine &amp; Data Structures</td>
                                    <td>Chosen over external frameworks for zero third-party dependencies, strict memory alignment, and predictable assembly generation.</td>
                                </tr>
                                <tr>
                                    <td><b>POSIX Threads (pthreads)</b></td>
                                    <td>Parallel Worker Threading</td>
                                    <td>Chosen over Boost.Asio for direct low-level control over CPU core pinning and thread affinity.</td>
                                </tr>
                                <tr>
                                    <td><b>Consistent 5-Tuple Hashing</b></td>
                                    <td>Thread Dispatch Router</td>
                                    <td>Chosen over round-robin load balancing to ensure all packets of one connection remain local to a single thread's flow table.</td>
                                </tr>
                                <tr>
                                    <td><b>Welford's Algorithm</b></td>
                                    <td>Flow Periodicity Statistics</td>
                                    <td>Chosen over sliding window timestamp arrays to calculate variance online in $O(1)$ constant time per packet.</td>
                                </tr>
                                <tr>
                                    <td><b>CMake</b></td>
                                    <td>Build System &amp; Test Suite</td>
                                    <td>Chosen for cross-platform C++ compilation and seamless integration with native GoogleTest unit test suites.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <section class="section" style="padding: 40px 0; border-bottom: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <span class="kicker-label" style="color: var(--stamp);">SYSTEM BENCHMARKS</span>
                        <h3 class="heading-lg" style="margin: 10px 0 20px 0;">Empirical Performance Metrics</h3>

                        <div class="metrics-card-grid">
                            <div class="metric-card">
                                <div class="metric-val-lg">ZERO</div>
                                <div class="metric-title">Hot-Path Lock Contention</div>
                                <div class="metric-caption">100% thread-isolated flow tables via deterministic 5-tuple consistent hashing.</div>
                            </div>

                            <div class="metric-card">
                                <div class="metric-val-lg">JA4+</div>
                                <div class="metric-title">TLS Fingerprinting</div>
                                <div class="metric-caption">Extracts ClientHello ciphers &amp; extensions while filtering GREASE noise.</div>
                            </div>
                            ...
                            <div class="metric-card">
                                <div class="metric-val-lg">ZERO</div>
                                <div class="metric-title">External Dependencies</div>
                                <div class="metric-caption">Built in pure C++17 STL for maximum portability and zero third-party library bloat.</div>
                            </div>
                        </div>
            ...
            <section class="section" style="padding: 20px 0;">
                <div class="newspaper-grid">
                    <div class="col-span-12">
                        <div class="decision-callout">
                            <span class="kicker-label" style="color: var(--stamp);">KEY ARCHITECTURAL DECISION</span>
                            <h3 class="heading-lg" style="margin: 10px 0 15px 0;">Lock-Free Threading via Deterministic 5-Tuple Hashing</h3>
            ...
            <section class="section" style="padding: 40px 0 20px 0; border-top: 1px solid var(--rule);">
                <div class="newspaper-grid">
                    <div class="col-span-12 text-center">
                        <h3 class="heading-lg" style="margin-bottom: 15px;">Explore the Source Code</h3>
                        <div style="margin-bottom: 25px;">
            ...
            <script>
                function playCipherAnim() {
                    const consoleEl = document.getElementById('cipherConsole');
                    const pktStream = document.getElementById('pktStream');
                    const ja4Match = document.getElementById('ja4Match');

                    consoleEl.innerHTML = '[0.0ms] [INGRESS] eBPF socket capturing raw packets at interface ingress...';

                    setTimeout(() => {
                        pktStream.innerHTML = `
                            [INGRESS] 10.0.4.12:49152 &rarr; 185.220.101.5:443 (TCP)<br>
                            &rarr; 5-Tuple Hash: <code style="color:#fef08a;">0x8f2c41a0</code> &rarr; <span style="color:#38bdf8; font-weight:bold;">Worker Thread #2 (Lock-Free)</span>
                        `;
                        consoleEl.innerHTML = '[0.2ms] [HASH ROUTER] Deterministically routed Flow <code>10.0.4.12:49152->185.220.101.5:443</code> to Thread #2. Mutex Contention: 0.';
                    }, 800);

                    setTimeout(() => {
                        ja4Match.innerHTML = `
                            Parsing ClientHello Handshake...<br>
                            Ciphers Sorted: <code>003c,002f,009c,009d</code><br>
                            JA4 Fingerprint: <code style="color:#f87171;">t13d190800_c84a8b291410</code>
                        `;
                        consoleEl.innerHTML = '[0.6ms] [JA4+ FINGERPRINT] Extracted ClientHello fingerprint. Filtering GREASE extensions...';
                    }, 1800);

                    setTimeout(() => {
                        ja4Match.innerHTML += `<br><span style="color:#ef4444; font-weight:bold;">[ALERT: CobaltStrike C2 Signature Matched]</span>`;
                        consoleEl.innerHTML = '[1.1ms] [WELFORD PERIODICITY] Flow Inter-Arrival CoV = <code>0.0024 (< 0.12)</code>. <b>MALICIOUS C2 BEACON DETECTED & KERNEL FILTERED</b>.';
                    }, 2800);
                }
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    if (window.gsap && window.ScrollTrigger) {
                        ScrollTrigger.create({
                            trigger: "#cipherAnimBox",
                            start: "top 75%",
                            onEnter: () => playCipherAnim()
                        });
                    }
                });
            </script>
