...
SENTINEL_CONTENT = """
            ...
            <!-- REAL WORK-LIFE SOC ANALYST SIMULATOR -->
            <section class="section" style="padding: 10px 0 30px 0;">
                ...
                <h3 class="heading-lg" style="margin: 5px 0 20px 0;">SOC Analyst Incident Response Workstation</h3>

                <div class="work-sim-container">
                    <div class="work-sim-header">
                        <div>
                            <span class="status-dot"></span>
                            <span style="font-weight:bold; color:#e2e8f0;">SENTINEL-STREAM // LIVE SOC ALERT CONSOLE</span>
                        </div>
                        <div style="display:flex; gap:10px;">
                            <span class="metric-badge">Memory: O(1) 1.98 KB</span>
                            <span class="metric-badge">Engine: LightGBM + IsoForest</span>
                        </div>
                    </div>

                    <p style="color:#94a3b8; font-size:13px; margin-bottom:15px;">Simulate real-world SOC Analyst triage actions against incoming enterprise threats:</p>

                    <div>
                        <button class="work-btn" onclick="simWork('triage')">🔍 Triage Active Alerts</button>
                        <button class="work-btn work-btn--danger" onclick="simWork('isolate')">🛡️ Isolate Compromised Host</button>
                        <button class="work-btn" onclick="simWork('shap')">📊 Run TreeSHAP Attribution</button>
                        <button class="work-btn" onclick="simWork('reset')">🔄 Clear Workstation</button>
                    </div>

                    <div class="console-output" id="socTerminal">
[SYSTEM MONITOR] SENTINEL-STREAM monitoring 50,000 active enterprise entity pipelines...
[SOC DESK] Standby for analyst triage command. Select an action button above...
                    </div>
                </div>
            </section>
            ...
            <script>
                function simWork(type) {
                    const term = document.getElementById('socTerminal');
                    const ts = new Date().toISOString().substring(11, 19);

                    let msg = '';
                    if (type === 'triage') {
                        msg = `[${ts}] [SOC TRIAGE ALERT] High-Priority Incident #SOC-9421 Detected\\n` +
                              `  -> Entity: dev_service_admin (IP: 10.0.12.84)\\n` +
                              `  -> Anomaly Score: 0.942 (CRITICAL) | P99 Latency: 2.14ms\\n` +
                              `  -> Pattern: 1,400 Database Queries in 10s + Outbound S3 Dump to unknown ASN\\n` +
                              `  -> Recommendation: Immediate Host Containment`;
                    } else if (type === 'isolate') {
                        msg = `[${ts}] [WORKSTATION ACTION] Host Isolation Triggered for dev_service_admin\\n` +
                              `  -> Network Quarantine Policy Pushed to EKS Security Group\\n` +
                              `  -> Active Session Revoked | OAuth Tokens Invalidated\\n` +
                              `  -> Incident Log #SOC-9421 Status: CONTAINED (Response Time: 4.2s)`;
                    } else if (type === 'shap') {
                        msg = `[${ts}] [TREESHAP ANALYSIS] Per-Alert Feature Risk Breakdown (#SOC-9421):\\n` +
                              `  1. outbound_bytes_ewma:  +0.64 (64% Risk Contribution)\\n` +
                              `  2. event_rate_delta:     +0.22 (22% Risk Contribution)\\n` +
                              `  3. off_hours_flag:       +0.08 (8% Risk Contribution)\\n` +
                              `  -> Decision Rationale Verified by SOC Lead Analyst`;
                    } else {
                        msg = `[${ts}] [SOC CONSOLE RESET] System Monitoring Resumed. Standby for events...`;
                    }
...
CIPHERPULSE_CONTENT = """
            ...
            <!-- REAL WORK-LIFE NETWORK FORENSIC SIMULATOR -->
            <section class="section" style="padding: 10px 0 30px 0;">
                <span class="kicker-label" style="color: var(--stamp); font-size: 14px;">REAL WORK-LIFE SIMULATION</span>
                <h3 class="heading-lg" style="margin: 5px 0 20px 0;">Network Forensic Incident Commander Workstation</h3>

                <div class="work-sim-container">
                    <div class="work-sim-header">
                        <div>
                            <span class="status-dot"></span>
                            <span style="font-weight:bold; color:#e2e8f0;">CIPHERPULSE // LINE-RATE FORENSIC CONSOLE</span>
                        </div>
                        <div style="display:flex; gap:10px;">
                            <span class="metric-badge">Capture: AF_PACKET / eBPF</span>
                            <span class="metric-badge">Engine: C++17 Ring Buffer</span>
                        </div>
                    </div>

                    <p style="color:#94a3b8; font-size:13px; margin-bottom:15px;">Simulate Network Security Engineer commands against encrypted malware beaconing:</p>

                    <div>
                        <button class="work-btn" onclick="simDpi('inspect')">🔍 Run JA4+ TLS Fingerprinting</button>
                        <button class="work-btn work-btn--danger" onclick="simDpi('block')">🚨 Block C2 Beaconing IP</button>
                        <button class="work-btn" onclick="simDpi('ring')">⚙️ Inspect Ring Buffer Locks</button>
                        <button class="work-btn" onclick="simDpi('clear')">🔄 Clear Workstation</button>
                    </div>

                    <div class="console-output" id="dpiTerminal">
[DPI SYSTEM] CipherPulse Fast-Path Workers listening on 4 CPU cores...
[FORENSIC DESK] Standby for Network Security Engineer command. Select an action button above...
                    </div>
                </div>
            </section>
            ...
            <script>
                function simDpi(type) {
                    const term = document.getElementById('dpiTerminal');
                    const ts = new Date().toISOString().substring(11, 19);

                    let msg = '';
                    if (type === 'inspect') {
                        msg = `[${ts}] [JA4+ ENGINE] Fingerprinted TLS 1.3 Handshake (#PKT-84910)\\n` +
                              `  -> JA4+ Hash: t13d151600_8daaf6152702_000000000000\\n` +
                              `  -> Target: 185.220.101.5:443 | SNI: HIDDEN (Encrypted Client Hello)\\n` +
                              `  -> Welford Flow CoV: 0.004 (< 0.12 Threshold)\\n` +
                              `  -> Incident Finding: CONFIRMED MALICIOUS C2 BEACONING`;
                    } else if (type === 'block') {
                        msg = `[${ts}] [FORENSIC ACTION] BPF Rule Injected to Kernel eBPF Map\\n` +
                              `  -> Dropping 185.220.101.5 at AF_PACKET Ingress Level\\n` +
                              `  -> C2 Beaconing Channel Nullified (Latency: 0.8ms)`;
                    } else if (type === 'ring') {
                        msg = `[${ts}] [RING BUFFER HEALTH] 4 Workers Active:\\n` +
                              `  -> Worker 0: 124,000 pps | Mutex Contention: 0.00%\\n` +
                              `  -> Worker 1: 124,000 pps | Mutex Contention: 0.00%\\n` +
                              `  -> Worker 2: 124,000 pps | Mutex Contention: 0.00%\\n` +
                              `  -> Worker 3: 124,000 pps | Mutex Contention: 0.00%\\n` +
                              `  -> Overall Throughput: 496,000 pps (Zero Packet Drops)`;
                    } else {
                        msg = `[${ts}] [FORENSIC CONSOLE RESET] Fast-Path Workers listening on 4 CPU cores...`;
                    }
...
print("Successfully generated clean, spacious, uncluttered pages with Real Work-Life Simulations!")
